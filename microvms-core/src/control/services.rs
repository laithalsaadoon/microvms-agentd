...
//!
//! The same reason the control plane is ([`super::transport`]): one HTTP stack, one TLS
...
//! ([`super::transport::default_credentials`]), so the ARN, the upload, and the create are
//! one identity.
//!
...
//! # The seam
//!
//! [`BuildServices`] is the trait `Sandbox::with_build_services` takes, the way
//! `ControlPlane::with_transport` takes a [`super::transport::Transport`]: a test records the
//! calls and answers them, and production builds [`SignedBuildServices`] on the first
//! `ensure_image`.
...
impl SignedBuildServices {
    ...
    pub async fn new(region: Region) -> Result<Self, Error> {
        Ok(Self {
            credentials: super::transport::default_credentials(&region).await?,
            http: super::transport::http_client(std::time::Duration::from_secs(15 * 60))?,
            region,
        })
    }
    ...
    async fn send(
        ...
        let credentials =
            super::transport::resolve_credentials(&self.credentials, operation).await?;
        super::transport::sign_for(&mut request, &credentials, &self.region, service, settings)?;
...
pub(crate) fn s3_object_url(bucket: &str, key: &str, region: &Region) -> String {
    let path: Vec<String> = key
        .split('/')
        .map(super::transport::paths::encode_segment)
        .collect();
    let path = path.join("/");
    if bucket.contains('.') {
        format!(

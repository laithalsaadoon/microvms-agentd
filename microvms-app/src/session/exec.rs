...
/// A fresh exec id for the wall time `unix`, since the epoch: `x-` plus 16 hex characters,
/// the Python client's shape.
///
/// The id needs to be distinct rather than unguessable — it is an idempotency key, not a
/// credential, and the daemon rejects an unknown one — so the nanosecond clock mixed with a
/// counter is enough. Code holding a [`crate::session::Session`] mints through the session's
/// clock, so a test that pins the clock pins the id. `microvms_core::session::mint_exec_id` is
/// this on the production clock.
///
/// Reads no clock: the time is the caller's. A process-wide counter still separates two ids
/// minted for the same instant.
pub fn exec_id_at(unix: Duration) -> String {
...
/// The id's shape, from its two inputs.
fn exec_id_from(nanos: u64, sequence: u64) -> String {
...
mod tests {
    ...
    /// The id is `x-` and sixteen hex characters of the time XOR the counter's high bits.
    #[test]
    fn an_exec_id_is_the_time_with_the_counter_in_its_high_bits() {
        assert_eq!(exec_id_from(0x1234, 0), "x-0000000000001234");
        assert_eq!(exec_id_from(0x1234, 1), "x-0000010000001234");
    }

    /// Two ids for the same instant still differ, which is what the counter is for.
    #[test]
    fn two_ids_for_one_instant_differ() {
        let instant = Duration::from_secs(1_767_225_600);
        assert_ne!(exec_id_at(instant), exec_id_at(instant));
    }

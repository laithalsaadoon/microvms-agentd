...
//! the cursor-driven stream. [`files`] is file and tar transfer. The sockets (the port
//! forwarder, the TCP tunnel, the interactive shell) and the reqwest backend are in
//! `microvms-edges`, and `microvms_core::session` re-exports them beside these.
...
pub struct Session {
    ...
    /// The wall time exec ids are minted from, and the clock the default proxy auth refreshes
    /// on.
    clock: Arc<dyn Clock>,
}
...
impl Session {
    ...
    ///
    /// `Session::connect`, `Session::attach` and `Session::direct` are in
    /// `microvms_core::prelude`, because they build the production HTTP backend (and `attach` a
    /// signed control plane), which is I/O this module leaves to its caller.
    ...
    /// A fresh exec id, minted from this session's clock.
    pub(crate) fn mint_exec_id(&self) -> String {
        exec_id_at(self.clock.unix_now())
    }
...
impl SessionBuilder {
    ...
    /// The clock exec ids are minted from, and the one a proxy auth built from
    /// [`Self::with_minter`] refreshes on. Required by [`Self::try_build`]; the adapters'
    /// clock under [`Self::build_with`].
    ///
    /// A [`ProxyAuth`] passed to [`Self::with_proxy_auth`] keeps its own clock.
    #[must_use]
    ...
    /// Builds the session from what was set, taking an unset backend and clock from
    /// `adapters`.
    ///
    /// A sandbox builds its session this way, with its plane's clock already set, so only the
    /// backend comes from the adapters. `SessionBuilder::build` (`microvms_core::prelude`) is this
    /// over the production adapters.
    pub fn build_with(
        mut self,
        adapters: &dyn crate::adapters::Adapters,
    ) -> Result<Session, Error> {
        if self.backend.is_none() {
            self.backend = Some(adapters.http_backend(&self.endpoint, self.timeout)?);
        }
        if self.clock.is_none() {
            self.clock = Some(adapters.clock());
        }
        self.try_build()
    }

    /// Builds the session from exactly what was set.
    ///
    /// Fails without a backend or a clock: this module has no production default for either,
    /// and a session that silently picked one would be a session a test can't pin.
    pub fn try_build(self) -> Result<Session, Error> {
        let (Some(backend), Some(clock)) = (self.backend, self.clock) else {
            return Err(Error::new(
                ErrorKind::Precondition,
                "a session needs a backend and a clock: set both with with_backend and \
                 with_clock, or build through the prelude's SessionBuilder::build",
            ));
        };
        let proxy = match (self.proxy, self.minter) {
            (Some(proxy), _) => Some(proxy),
            (None, Some(minter)) => Some(Arc::new(ProxyAuth::with_clock(
                minter,
                self.port,
                Arc::clone(&clock),
            ))),
...
pub(crate) mod testing {
    ...
    impl Recorder {
        pub fn with(replies: impl IntoIterator<Item = Reply>) -> Arc<Self> {
            Arc::new(Self {
    ...
    pub fn session_with(
        ...
        Session,
        Arc<ProxyAuth>,
        Arc<crate::clock::testing::TestClock>,
    ) {
        let clock = Arc::new(crate::clock::testing::TestClock::default());
        let auth = Arc::new(
    ...
    pub fn health_body(bootstrapped: bool) -> serde_json::Value {
        serde_json::json!({
            "version": "0.1.0",
...
mod tests {
    ...
    /// `try_build` refuses a builder with no backend or no clock rather than picking one:
    /// this module has no production default, and a silent one would be a session a test
    /// can't pin.
    ///
    /// **Falsification**: default the clock to a `TestClock` in `try_build` and the
    /// clockless builder below builds.
    #[test]
    fn try_build_needs_a_backend_and_a_clock() {
        let clock = Arc::new(crate::clock::testing::TestClock::new()) as Arc<dyn Clock>;
        let no_backend = Session::builder("https://vm.example", "token")
            .with_clock(Arc::clone(&clock))
            .try_build()
            .expect_err("no backend");
        assert_eq!(no_backend.kind(), ErrorKind::Precondition);
        let no_clock = Session::builder("https://vm.example", "token")
            .with_backend(Recorder::with([]))
            .try_build()
            .expect_err("no clock");
        assert_eq!(no_clock.kind(), ErrorKind::Precondition);
        Session::builder("https://vm.example", "token")
            .with_backend(Recorder::with([]))
            .with_clock(clock)
            .try_build()
            .expect("both set");
    ...
    /// `build_with` fills only what's unset: a backend the caller set wins over the
    /// adapters' one.
    ///
    /// **Falsification**: take the adapters' backend unconditionally and the request below
    /// goes to the inert test backend, which refuses it.
    #[tokio::test]
    async fn build_with_keeps_the_callers_backend() {
        ...
        let session = Session::builder("http://127.0.0.1:9000", "token")
            ...
            .expect("builds");
        session.health().await.expect("the recorder answered");
    ...
    async fn a_mint_failure_surfaces_as_a_retryable_request_error() {
        ...
        let auth = Arc::new(
            ProxyAuth::with_refresh_after(
                ...
                DEFAULT_AGENT_PORT,
                DEFAULT_REFRESH_AFTER,
                Arc::new(proxy::testing::TestClock::default()),
            )

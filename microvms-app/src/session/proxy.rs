...
/// The clock a refresh boundary is measured on. It's the crate's one [`Clock`], and the
/// production one is tokio's (`TokioClock`, in `microvms-edges`): a token timed on
/// `std::time::Instant` would never re-mint inside a simulation.
...
impl ProxyAuth {
    /// A [`ProxyAuth`] with the default refresh interval, on `clock`.
    ///
    /// Infallible where [`Self::with_refresh_after`] isn't: [`DEFAULT_REFRESH_AFTER`] is a
    /// constant this module owns, and a test below asserts it satisfies that guard.
    /// `ProxyAuth::new(minter, port)` is this on tokio's clock, in `microvms_core::prelude`.
    pub fn with_clock(minter: Arc<dyn TokenMinter>, port: u16, clock: Arc<dyn Clock>) -> Self {
        Self {
            minter,
            port,
...
pub(crate) mod testing {
    ...
    #[cfg(test)]
    pub(crate) use crate::clock::testing::TestClock;
    ...
    impl CountingMinter {
        ...
        pub fn without_auth_header() -> Self {
            Self {
...
#[cfg(test)]
mod tests {
    use super::testing::{CountingMinter, TestClock};
    use super::*;
    use crate::error::ErrorKind;

    fn auth(clock: Arc<TestClock>, refresh_after: Duration) -> ProxyAuth {
    ...
    /// which is what lets [`ProxyAuth::with_clock`] be infallible.
    ///
    ...
    async fn every_header_set_carries_both_the_auth_and_the_port_header() {
        let auth = auth(Arc::new(TestClock::default()), DEFAULT_REFRESH_AFTER);
    ...
    async fn concurrent_requests_on_a_cold_cache_mint_once() {
        let auth = Arc::new(auth(Arc::new(TestClock::default()), DEFAULT_REFRESH_AFTER));
    ...
    async fn headers_for_a_port_are_backed_by_a_token_minted_for_that_port() {
        let (auth, minter) = auth_with(Arc::new(TestClock::default()), DEFAULT_REFRESH_AFTER);

...
//! constructed state: `ControlPlane::new` (`microvms_core::prelude::ControlPlaneExt`) resolves
//! credentials and either yields a usable client or says why not.
//!
...
pub use services::BuildServices;
...
/// The clock every wait here measures its deadline on. It's the crate's one [`Clock`], so the
/// plane and the sessions it hands out read the same time source.
pub use crate::clock::Clock;
use crate::entropy::Entropy;

/// The control-plane client.
///
/// The transport, clock, entropy and adapters sit behind `Arc`, so a caller holding one across
/// tasks does not need a second credential chain.
pub struct ControlPlane {
    transport: Arc<dyn transport::Transport>,
    region: Region,
    clock: Arc<dyn Clock>,
    /// Where client-token nonces, agent tokens and identity seeds come from.
    entropy: Arc<dyn Entropy>,
    /// What a sandbox over this plane builds its session backend and its STS and S3 client
    /// from.
...
impl ControlPlane {
    /// A client over caller-supplied ports, on the default agent port.
    ///
    /// Every port is explicit, including the entropy. A plane over a scripted transport
    /// still mints client tokens, and a deterministic default would mint repeating ones
    /// (TRAP-1), so nothing here picks a source for the caller.
    ///
    /// `ControlPlane::new(region)` and `ControlPlane::with_transport(transport, region, clock)`
    /// are in `microvms_core::prelude`: they wire the production transport, clock, entropy and
    /// adapters, which is I/O this constructor leaves to its caller. A test builds one with
    /// `testing::control_plane` instead.
    pub fn from_ports(
        transport: Arc<dyn transport::Transport>,
        region: Region,
        clock: Arc<dyn Clock>,
    ...
    /// Replaces the source client-token nonces, agent tokens and identity seeds are drawn
    /// from. The seam a test pins those values through.
    #[must_use]
    ...
    /// The source this plane draws client-token nonces, agent tokens and identity seeds from.
    ///
    /// Public so the composition root's wiring can be checked from outside this crate: a
    /// production plane must carry the OS pool, never a deterministic source (TRAP-1).
    pub fn entropy(&self) -> &dyn Entropy {
        self.entropy.as_ref()
    }

    /// The clock itself, for a session that has to read the same one.
    pub(crate) fn shared_clock(&self) -> Arc<dyn Clock> {
        Arc::clone(&self.clock)
    }

    pub(crate) fn adapters(&self) -> &dyn Adapters {
...
mod tests {
    ...
    fn the_wire_types_token_field_is_crate_private_and_minted() {
        ...
        let wire = ops::RunMicrovmWire {
            ...
            client_token: token::run_token_with("arn:image", &entropy).expect("scripted"),
        };
        assert!(wire.client_token.starts_with("run-"));
        assert!(wire.client_token.len() <= crate::constants::MAX_CLIENT_TOKEN_LEN);
        assert_ne!(
            wire.client_token,
            token::run_token_with("arn:image", &entropy).expect("scripted"),

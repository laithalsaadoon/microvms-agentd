...
//! [`create_token_with`] and [`run_token_with`] take a **scope label** and mint the token
...
/// An image-create idempotency token, unique per attempt, with its nonce drawn from `entropy`.
///
...
/// (`docs/PLATFORM.md`, "`clientToken` is a permanent idempotency key"). An unavailable
/// source is an error. `microvms_core::control::token::create_token` is this over the OS
/// pool, under the name it had.
pub fn create_token_with(scope: &str, entropy: &dyn Entropy) -> Result<String, Error> {
    Ok(mint(Verb::Create, scope, &nonce_from(entropy)?))
}

/// A run idempotency token, unique per attempt, with its nonce drawn from `entropy`. Same
/// rule as [`create_token_with`].
///
...
/// the same guest. `microvms_core::control::token::run_token` is this over the OS pool.
pub fn run_token_with(scope: &str, entropy: &dyn Entropy) -> Result<String, Error> {
    Ok(mint(Verb::Run, scope, &nonce_from(entropy)?))
}
...
/// [`create_token_with`] folds into an idempotency token, reused rather than reinvented.
///
...
pub(crate) fn resolve_log_stream(prefix: &str, entropy: &dyn Entropy) -> Result<String, Error> {
    let nonce = nonce_from(entropy)?;
...
/// Eight bytes of fresh randomness per call, from `entropy`.
///
/// TRAP-1 needs per-attempt distinctness — the token must not repeat across two
/// attempts; it does not have to be unguessable. The OS pool gives the stronger
/// property anyway, and fails only when it is genuinely unavailable, in which state a
/// clock-derived nonce (the old fallback here) risks the exact collision TRAP-1 exists
/// to prevent. So an unavailable source is an error, never a fallback.
fn nonce_from(entropy: &dyn Entropy) -> Result<[u8; TOKEN_NONCE_BYTES], Error> {
...
mod tests {
    ...
    /// One source for the whole module, so every token a test here mints is a fresh draw. The
    /// OS pool's own distinctness is `microvms-edges`'s `the_os_source_draws_distinct_bytes`.
    static SOURCE: LazyLock<SequenceEntropy> = LazyLock::new(SequenceEntropy::new);

    fn create_token(scope: &str) -> String {
        create_token_with(scope, &*SOURCE).expect("scripted")
    }

    fn run_token(scope: &str) -> String {
        run_token_with(scope, &*SOURCE).expect("scripted")
    }
    ...
    /// 200 distinct values.
    ///
    /// **Falsification** — replace `nonce_from()` in `create_token_with` with a digest of
    ...
    fn a_resolved_log_stream_carries_the_prefix_a_slash_and_sixteen_fresh_hex() {
        let first = resolve_log_stream("ci-image", &*SOURCE).expect("the pool is available");
        let second = resolve_log_stream("ci-image", &*SOURCE).expect("the pool is available");
    ...
    fn the_longest_legal_prefix_resolves_to_exactly_the_shapes_ceiling() {
        ...
        let resolved = resolve_log_stream(&prefix, &*SOURCE).expect("the pool is available");
        assert_eq!(resolved.len(), crate::constants::MAX_LOG_STREAM_LEN);
    }

    /// The randomness source produces distinct draws. A `nonce_from` that returned a
    ...
    fn the_nonce_source_does_not_repeat_itself() {
        let drawn: HashSet<[u8; TOKEN_NONCE_BYTES]> = (0..200)
            .map(|_| nonce_from(&*SOURCE).expect("the pool is available"))
            .collect();
    ...
    /// Every nonce is the source's draw, verbatim, so a control plane's source decides it.
    ///
    /// **Falsification**: draw from `OsEntropy` inside `nonce_from` and the three values
    /// below stop being the scripted draws.
    #[test]
    fn the_nonce_is_the_source_s_next_draw() {
        let entropy = SequenceEntropy::new();
        let hex = |n| const_hex::encode(SequenceEntropy::draw(n, TOKEN_NONCE_BYTES));
        assert_eq!(
            create_token_with("img", &entropy).expect("scripted"),
            format!("create-img-{}", hex(1))
        );
        assert_eq!(
            run_token_with("img", &entropy).expect("scripted"),
            format!("run-img-{}", hex(2))
        );
        assert_eq!(
            resolve_log_stream("ci", &entropy).expect("scripted"),
            format!("ci/{}", hex(3))
        );
    }

    /// An unavailable source refuses the token rather than minting one from anything else.
    #[test]
    fn an_unavailable_source_refuses_every_token() {
        let entropy = SequenceEntropy::unavailable();
        assert!(create_token_with("img", &entropy).is_err());
        assert!(run_token_with("img", &entropy).is_err());
        assert!(resolve_log_stream("ci", &entropy).is_err());
    }
}

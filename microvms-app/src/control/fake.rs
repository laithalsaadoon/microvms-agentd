...
struct Inner {
    ...
    /// What resolving credentials fails with, or `None` for a chain that resolves.
...
impl FakeControlPlane {
    ...
    /// Makes resolving credentials fail with a `Credentials` error carrying `message`, the
    /// way a default chain that finds nothing does.
    ///
    /// Calls are still recorded and answered from the queue, so a test asserts that none
    /// followed the failure by reading [`Self::calls`], not by the fake refusing them.
    pub fn fail_credentials(&self, message: impl Into<String>) -> &Self {
        self.inner.lock().expect("not poisoned").credentials_error = Some(message.into());
        self
    }
...
impl Transport for FakeControlPlane {
    ...
    fn resolve_credentials(
        ...
        let result = match &self.inner.lock().expect("not poisoned").credentials_error {
            Some(message) => Err(Error::new(ErrorKind::Credentials, message.clone())),
            None => Ok(()),
        };
        Box::pin(async move { result })
    }
...
/// The crate's shared test clock, where the control-plane tests have always found it.
#[cfg(test)]

...
impl ControlPlane {
    ...
    pub async fn create_image(&self, request: CreateImageRequest) -> Result<Image, Error> {
        ...
        let resolved_stream = request
            .log_stream
            .as_deref()
            .map(|prefix| token::resolve_log_stream(prefix, self.entropy()))
            .transpose()?;
        let logging = request
            .log_group
            .as_deref()
        ...
        let wire = ops::CreateMicrovmImageWire {
            ...
            client_token: token::create_token_with(
...
mod tests {
    ...
    fn planted() -> (ControlPlane, Arc<FakeControlPlane>, Arc<TestClock>) {
        ...
        let plane = crate::testing::control_plane(fake.clone(), Region::UsEast1, clock.clone());
        (plane, fake, clock)
    }
    ...
    /// **A create draws its log-stream discriminator and its client-token nonce from the
    /// plane's entropy**, stream first, so a swapped source decides both.
    ///
    /// **Falsification**: resolve either through `OsEntropy` instead of `self.entropy()` and
    /// its assertion reads a value the scripted source never produced.
    #[tokio::test]
    async fn a_create_draws_its_random_values_from_the_planes_entropy() {
        ...
        .with_entropy(Arc::new(SequenceEntropy::new()));
        fake.answer(
            "CreateMicrovmImage",
            Answer::created(fake::create_image_response("img")),
        );
        ...
        let body = fake.first_body("CreateMicrovmImage");
        let draw = |n| const_hex::encode(SequenceEntropy::draw(n, 8));
        assert_eq!(
            body["logging"]["cloudWatch"]["logStream"],
            format!("ci-image/{}", draw(1))
        );
        assert_eq!(
            body["clientToken"],
            format!("create-agentd-conformance-{}", draw(2))
        );
    }

    /// An unavailable source refuses the create before anything reaches the plane.
    #[tokio::test]
    async fn an_unavailable_entropy_source_refuses_the_create() {
        use crate::entropy::testing::SequenceEntropy;
        let fake = Arc::new(FakeControlPlane::new());
        let plane = crate::testing::control_plane(
            fake.clone(),
        ...
        let err = plane
            ...
            .await
            .expect_err("no pool, no client token");
        assert_eq!(err.kind(), crate::error::ErrorKind::Unexpected, "{err}");
    ...
    /// port is 0 cannot exist: [`ControlPlane::from_ports`] takes the default. A

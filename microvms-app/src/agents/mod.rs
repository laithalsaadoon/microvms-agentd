...
///
/// `mint` supplies the exec id when `options` carries none. [`prompt`] mints from its
/// session's clock; `microvms_core::agents::prompt_request` is this with the production one.
pub fn prompt_request_with(
    spec: &AgentSpec,
    task: &str,
...
pub async fn prompt(
    ...
    session
        .run(prompt_request_with(spec, task, options, || {
...
impl AgentVm {
    ...
    ///
    /// `AgentVm::from_name` and `AgentVm::adopt_in`, which resolve credentials for a region,
    /// are in `microvms_core::prelude`.
    ///
...
mod tests {
    ...
    /// [`prompt_request_with`] minting from a fixed instant, the shape the bindings' default
    /// has without the production clock.
    fn prompt_request(
        spec: &AgentSpec,
        task: &str,
        options: &PromptOptions,
    ) -> Result<crate::protocol::exec::StartRequest, Error> {
        prompt_request_with(spec, task, options, || {
            crate::session::exec_id_at(Duration::from_secs(1_767_225_600))
        })
    }
    ...
    fn access() -> BedrockAccess {
        BedrockAccess {
            ...
            // Token-shaped and never signed: nothing here sends it anywhere.
            token: bedrock::BearerToken::new("bedrock-api-key-test".to_string()),
        }
    }
    ...
    fn sandbox() -> Sandbox {
        Sandbox::with_control_plane(crate::testing::control_plane(
    ...
    /// An agent helper's exec id is minted from the session's clock, not the host's.
    ///
    /// The id's low 40 bits are the wall time's nanoseconds (the counter folds into the high
    /// bits), so they pin which clock the id was read from.
    ///
    /// **Falsification**: mint the probe's id with the free `mint_exec_id()` and the low bits
    /// are the host's nanoseconds instead of the pinned instant's.
    #[tokio::test]
    async fn an_agent_exec_id_is_minted_from_the_sessions_clock() {
        let recorder = Recorder::with([
            Reply::ok(serde_json::json!({"exec_id":"version","phase":"running"})),
            Reply::ok(serde_json::json!({"exec_id":"version","phase":"exited"})),
            Reply::ok(serde_json::json!({
                "exec_id":"version","phase":"exited","exit_code":0,"signal":null,
                "stdout":"codex-cli 0.154.0","stderr":"",
                "truncated":false,"writers_may_be_alive":false
            })),
        ]);
        let (session, _, clock) = session_with(Arc::clone(&recorder));
        let pinned = Duration::from_secs(1_767_225_600) + Duration::from_nanos(123_456_789);
        clock.set_unix_now(pinned);
        installed_version(&session, Agent::Codex).await.unwrap();

        let start: serde_json::Value =
            serde_json::from_slice(&recorder.requests()[0].body).unwrap();
        let id = start["exec_id"].as_str().expect("an exec id");
        let value =
        ...
        assert_eq!(
            ...
            "{id} wasn't minted from the session's clock"
        );
    }

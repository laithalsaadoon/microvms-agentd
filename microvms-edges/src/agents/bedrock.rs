...
//! forbids both there. The signer does I/O (the credential chain), so it's here in the edges;
//! the token and its lifetime are `microvms_app::agents::bedrock`.
//!
...
use microvms_app::agents::bedrock::{BearerToken, MAX_LIFETIME, Minted};

...
Defined at: `microvms-domain/src/error.rs:43` (`Error`), `:127` (`ErrorKind`, variants enumerated at
`:166`), `:219` (`WireKind`, variants enumerated at `:272`).

Gate: `microvms-domain/src/error.rs:433 every_kind_carries_its_python_err_code` — every kind must carry
...
(`microvms-domain/src/error.rs:18`). `microvms-domain/src/error.rs` itself holds 140 references; the heaviest consumers
follow.
...
- **`ErrorKind` is derived from `WireKind` and must never be passed in beside it.**
  `microvms-domain/src/error.rs:77 Error::wire` computes the kind from the wire kind precisely so the
...
- **`from_status` has no generic 4xx fallback, and that absence is a tested invariant.**
  `microvms-domain/src/error.rs:343` maps the statuses explicitly, and
  ...
  excepted as `NotBootstrapped` (`microvms-domain/src/error.rs:539`).
- **The retryable set is derived from the kind, never stored, and the test compares it against an
  independently restated table.** `microvms-domain/src/error.rs:116 Error::retryable` reads
...
| `microvms-domain/src/error.rs` | indirect | yes | `ErrorKind::ALL` (`:166`) and `ErrorKind::code` (`:187`) are the other half; `microvms-cli/src/exit.rs:486` asserts the two describe the same classes |
...
Defined at: `microvms-domain/src/constants.rs:57`-`:455` (the constants, from `MODEL_API_VERSION` to
...
- **These guards exist because botocore does not enforce the limits itself.**
  `microvms-domain/src/constants.rs:14` records the measurement: `VALIDATED_METADATA_ATTRS` is
...
- **`DEAD_STATES` is a strict subset of `TERMINAL_STATES`, and `SUSPENDED` must stay out of it.**
  `microvms-domain/src/constants.rs:448` lists the terminal states `TERMINATED`, `TERMINATING`,
...
- **The two image-ready sets must stay disjoint, or the gate reports a tolerated spelling as
  model-backed.** `microvms-domain/src/constants.rs:431` is checked against the model exactly and
...
In-crate, `microvms-domain/src/sizing.rs:273 the_documented_table_carries_the_measured_rows` pins the
rows.
...
- **Every shipped peak is exactly 4x its baseline, and nothing may compute it.**
  `microvms-domain/src/sizing.rs:15` names computing the peak as the one thing the module must not do.
...
- **An off-table baseline is refused rather than snapped, and the refusal is a billing decision.**
  `microvms-domain/src/sizing.rs:146 from_baseline_mib` rejects anything not in the table, asserted at
...
- **Two of the numbers are each both a baseline and a peak, so a caller echoing back a `MemTotal`
  gets a different class silently.** `microvms-domain/src/sizing.rs:384 a_peak_that_is_not_also_a_baseline_is_refused`
...
botocore calls that look like substitutes disagree with each other (`microvms-domain/src/region.rs:21`).
...
- **`unlisted()` normalizes a supported name back to its variant, so there is never a second spelling
  of one region.** `microvms-domain/src/region.rs:107`, asserted at
...
- **`supported()` is the single reader of the five spellings, and both `FromStr` and `unlisted` go
  through it.** `microvms-domain/src/region.rs:119`. A second lookup table added anywhere, including in
...
Defined at: `microvms-domain/src/cost.rs:1018` (`pinned_rates`), returning the `RateTable` declared at
...
(`microvms-domain/src/cost.rs:999`). One of them, `storage_gb_month`, is derived rather than read.
...
| `microvms-cli/src/render.rs` | direct import | likely | `:399` reads `retrieved()` (`microvms-domain/src/cost.rs:885`) for the report header; the remaining uses are under `#[cfg(test)]` from `:404` |
| `microvms-domain/src/cost.rs` (own tests) | test | yes | `:2180` pins every figure as a literal; `:2205` asserts the GB-month derivation as `dec!(0.0001111111) * dec!(730)` |
| `conformance/run_rs.py` | test | likely | asserts the `cost` command and the run envelope each report a labelled estimate |
| `docs/PLATFORM.md` | config | yes | `:293`, `:295`, and `:299` carry the same figures; `:304`-`:306` carry the GB-hour → GB-month derivation. `microvms-domain/src/cost.rs:57` and `:1002` both point here, so the two change in one commit |
...
- `agentd/src/routes.rs:371 surface_docs()` — the single route list the router (`:31`), the
  ...
  it does not exist.
- `microvms-domain/src/hooks.rs:48 RunHookTimeout` / `:54 BuildHookTimeout` — two newtypes with no

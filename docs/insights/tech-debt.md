...
| 5 | The committed `Cargo.lock` at `5e6f752` pins `h2` 0.4.15, which carries a RUSTSEC advisory patched in 0.4.16. `cargo deny check` runs in `mise run security` and in CI, so the gate is red at HEAD; it is green here only because of an uncommitted `cargo update -p h2` → 0.4.19 in the working tree. That update also moved `windows-sys` 0.61.2 → 0.52.0 in three places, a transitive downgrade nobody asked for — evidence that a targeted lockfile bump is not a targeted change. | version pin | S | `Cargo.lock:1576-1577`, `mise.toml:118`, `.github/workflows/ci.yml:173-177` |

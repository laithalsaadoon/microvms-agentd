...
findings the earlier passes did not. Every count on this page was measured at `5e6f752`.
...
| 12 | `deny.toml`'s `[advisories]` block asserts a delegation its own config version cannot express: "Advisory scanning is grype/trivy/osv-scanner's lane … Running RustSec here too would give findings a fourth place to be silenced." Under `version = 2` cargo-deny has no key that turns vulnerability scanning off — the v1 `vulnerability = "allow"` field was removed. Verified this session with cargo-deny 0.20.2: `cargo deny check advisories` reports `advisories ok` and exits 0, so the check is live. That mismatch is why row 5's advisory surfaced as a `cargo deny` failure rather than as a scanner finding. | deprecated pattern | S | `deny.toml:50-57`, `mise.toml:118` |
...
- `//! # Runtime-checked rather than typestate, deliberately` — `microvms-app/src/sandbox.rs:19`
...
- `//! # TRAP-11: what is deliberately absent` — `microvms-app/src/control/connector.rs:16`
- `/// # What is deliberately not here` — `microvms-app/src/control/mod.rs:216`
- `//! # What is deliberately *not* an option` — `microvms-cli/src/cli.rs:21`
- `//! # Hand-rolled rather than a dependency` — `microvms-app/src/session/sse.rs:16`
...
`cargo test --all` runs zero tests against 7,616 lines of them at `5e6f752`. The Node side has no lockfile,
...
"twice the", "four times", "matching the", "same as the daemon's". The pairs found that way got
...
and `agentd` writes the literal a third time in `Config::default`. The tests that mention
...
- `microvms-app/src/control/mod.rs:96-97` and `microvms-app/src/session/proxy.rs:82-83` — the same constant declared twice in one crate
- `agentd/src/config.rs:84` — the third copy, as a bare literal in the type that owns the truth
- `microvms-app/src/control/artifact.rs:395-430` — the pair that *was* guarded, with equality refused and the failure naming both numbers
...
- `.erpaval/solutions/architecture-patterns/an-absent-value-is-not-a-neutral-one.md:39-48` — the repo's own detection method, which found the guarded pairs and has not been re-run
